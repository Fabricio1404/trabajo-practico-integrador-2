...
export async function createComment(req, res) {
  try {
    ...
    if (!content || !author || !article) {
      return res
        .status(400)
        .json({ error: 'content, author y article son obligatorios' });
    }
    ...
    if (!user) {
      return res
        .status(400)
        .json({ error: 'author inválido (usuario no existe)' });
    }

    if (!art) {
      return res
        .status(400)
        .json({ error: 'article inválido (artículo no existe)' });
    }
...
export async function listCommentsByArticle(req, res) {
  try {
    ...
    return res.status(500).json({
      error: 'Error listando comentarios',
      detail: err.message,
    });
  }
}
...
export async function updateComment(req, res) {
  try {
    ...
    return res.status(500).json({
      error: 'Error actualizando comentario',
      detail: err.message,
    });
  }
}
